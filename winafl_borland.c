...
pre_fuzz_handler(void *wrapcxt, INOUT void **user_data)
{
    ...
    if (!options.no_loop) {
        if (fuzz_target.iteration == 0) {
            ...
            options.func_args[0] = mc->eax;
            options.func_args[1] = mc->edx;
            options.func_args[2] = mc->ecx;
        } else {
            /*for (i = 0; i < options.num_fuz_args; i++)
                drwrap_set_arg(wrapcxt, i, options.func_args[i]);
            */
            //mc->eax = options.func_args[0];
            //mc->edx = options.func_args[1];
            //mc->ecx = options.func_args[2];
        }
    }
...
event_module_load(void *drcontext, const module_data_t *info, bool loaded)
{
    ...
    if (_stricmp(module_name, "user32.dll") == 0) {
        to_wrap = (app_pc)dr_get_proc_address(info->handle, "WaitMessage");
