...
bool is_open_testcase_file = false;
                      ...
                      bool for_trace, bool translating, void *user_data)
{
    ...
    if(options.debug_mode)
      dr_fprintf(winafl_data.log, "mod_entry = 0x%x, mod_entry->data = 0x%x\n", mod_entry, mod_entry->data);
    if (mod_entry == NULL || mod_entry->data == NULL) return DR_EMIT_DEFAULT;

    module_name = dr_module_preferred_name(mod_entry->data);
    if(options.debug_mode)
      dr_fprintf(winafl_data.log, "module: %s\n", module_name);
    ...
    if(options.debug_mode)
      dr_fprintf(winafl_data.log, "%lx %lx\n", mod_entry->data->start, start_pc);
...
pre_fuzz_handler(void *wrapcxt, INOUT void **user_data)
{
    ...
    /*
    FILE *f;
    f = fopen("__create_handle__","a");
    fprintf(f, "pre_fuzz_handler\n");
    fclose(f);
    f = fopen("__close_handle__","a");
    fprintf(f, "pre_fuzz_handler\n");
    fclose(f);
    */
...
post_fuzz_handler(void *wrapcxt, void *user_data)
{
    ...
    */
    ...
    {
      if(options.debug_mode)
        dr_fprintf(winafl_data.log, "CloseHandle(0x%lx)\n", file_handler);
...
createfilew_interceptor(void *wrapcxt, INOUT void **user_data)
{
    ...
    if( wcsstr(filenamew, L".cur_input") )
      is_open_testcase_file = true;
}
static void
createfilew_interceptor_post(void *wrapcxt, INOUT void **user_data)
{
    int handle;
    handle = (int)drwrap_get_retval(wrapcxt);
    if(options.debug_mode)
        dr_fprintf(winafl_data.log, "In OpenFileW, handle 0x%lx\n", handle);

    if(is_open_testcase_file && handle != 0xffffffff && file_handler == 0)
    {
      is_open_testcase_file = false;
      file_handler = (HANDLE)handle;
      /*
      FILE *f;
      f = fopen("__create_handle__","a");
      fprintf(f, "%lx\n", file_handler);
...
static void closehandle_interceptor(void *wrapcxt, INOUT void **user_data)
{
    HANDLE handle = (HANDLE)drwrap_get_arg(wrapcxt, 0);
    if(0 && handle == file_handler)
    {
      file_handler = 0;
      /*
      FILE *f;
    ...
    if(options.debug_mode)
      dr_fprintf(winafl_data.log, "In CloseHandle, closing 0x%lx\n", handle);
}
...
static void messagebox_interceptor(void *wrapcxt, INOUT void **user_data)
{
  unsigned long int status = 0;
  drwrap_skip_call(wrapcxt, (void *)status, 4);
}
...
event_module_load(void *drcontext, const module_data_t *info, bool loaded)
{
    ...
    if(options.debug_mode)
        dr_fprintf(winafl_data.log, "Module loaded, %s in 0x%lx\n", module_name, info->start);
    ...
    if(options.fuzz_module[0]) {
        ...
        if(strcmp(module_name, "KERNEL32.dll") == 0) {
            ...
            if(options.debug_mode)
              dr_fprintf(winafl_data.log, "CreateFileW in 0x%lx\n", to_wrap);
    ...
    if(strcmp(module_name, "KERNEL32.dll") == 0) {
        to_wrap = (app_pc)dr_get_proc_address(info->handle, "CloseHandle");
        drwrap_wrap(to_wrap, closehandle_interceptor, NULL);
    }
...
options_init(client_id_t id, int argc, const char *argv[])
{
    ...
    for (i = 1/*skip client*/; i < argc; i++) {
        ...
        if (strcmp(token, "-no_nudge_kills") == 0)
            ...
            dr_fprintf(winafl_data.log, "target_module is %s\n", argv[i]);
            dr_fprintf(winafl_data.log, "fuzz_module is %s\n", options.fuzz_module);
        }
